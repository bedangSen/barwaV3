...
var textToSpeech = new TextToSpeechV1({
    username: '49bb5c77-ae6a-41cd-9bee-6937de347056',
    password: 'mo5v0aTwUCJR',
 ...
 request({
        ...
        if (!error && response.statusCode == 200) {
          ...
          if(body.docs[0]== null)
          {
            ...
            for (let i=0; i<4; i++)
            {
              //console.log("latitude: ",body.docs[i].latitude," , Longitude: ", body.docs[i].longitude, " , time: ", body.docs[i].time);
                  const LocUrl =
                    "https://maps.googleapis.com/maps/api/geocode/json?latlng="+body.docs[i].latitude+","+body.docs[i].longitude+"&key=AIzaSyAqkf8A7DYeNWIZSozQkC5ZnyA5NeUVKlQ+";
                  const timeVariable = body.docs[i].time;


                    request.get(LocUrl, (error, response, body) => {
                            let json = JSON.parse(body);
                           console.log('Location is at = ',json.results[i],'\n');

                            const LocaN ="Location: "+ json.results[i].formatted_address+ ", Time: "+ timeVariable
                           //
                            payload.context['N_LocaN'] = LocaN
                           //
                            payload.context['N_LocationN'] = json.results[i].formatted_address
...
//   let json = JSON.parse(body);
// //  console.log(json.results[1].address_components[0].long_name);
// console.log("JOKES ---------- JOKES: "+json);
//
// payload.context['joke_n'] = json
//
// //console.log(json.results[1].address_components[0].long_name);
//
//
...
request(options, (err, response, body) => {
    if(!err && response.statusCode == 200)
     console.log("JOKES ---------- JOKES: "+body);

      payload.context['joke_n'] = body
                ...
                //       })
                      ...
                      //  console.log('personality emotion ====== ', payload.context['emotion_score']);
                        ...
                        conversation.message(payload, function(err, data) {
                            ...
                            console.log("response---====    ",JSON.stringify(data.output.text[0], null, 2));
                                                       var outputText = JSON.stringify(data.output.text[0])
                                                       var wav = require('wav');
                                                      ...
                                                      //
                                                      //
                                                        textToSpeech.synthesize({ text: outputText, accept: 'audio/wav' }).pipe(reader);
